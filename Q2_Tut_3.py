# -*- coding: utf-8 -*-
"""
Created on Mon May  7 05:35:52 2018
...
#Dictionary entries
	value = {'no. of particles': 11, 'Gravitational constant': 6.67*10**-11}

	def PotentialEnergy(self,m,x,y):
             t = numpy.zeros(len(x))
			...
			for j in range(0,len(x)):
				t[j] = numpy.sqrt((x[r]-x[j])**2 + (y[r]-y[j])**2)
...
if __name__=="__main__":
        ...
        print ('The y position  ' + repr(Test.yposition),  "with potentials")

        Energy = Test.PotentialEnergy(Test.masses,Test.xposition,Test.yposition)
